const testimonials = [
  {
    ...
    avatar: "https://i.ibb.co/zZdShGy/img-photo.png",
  },
  {
    rating: "⭐️⭐️⭐️⭐️⭐️",
    title: "Lorem ipsum dolor sit amet",
    ...
    card.innerHTML = `
      <div class="card py-2" style="background: #F1F3FF">
        <div class="card-body d-flex flex-column flex-lg-row justify-content-between align-items-center px-2 pl-3 pl-lg-4">
          <div class="px-3">
            <img src="${testimonial.avatar}" alt="avatar" width="80" style="border-radius: 50%;"/>
          </div>
          <div >
            <h5 class="card-title">${testimonial.rating}</h5>
            <p class="card-text" style="white-space: wrap;">${testimonial.content}</p>
            <p class="card-text" style="white-space: wrap; font-weight: bold;">${`John Dee 32, Bromo`}</p>
          </div>
        </div>
      </div>
    `;
...
renderTestimonial(document.querySelector("#card-testimonial"));
renderWhyus(document.querySelector("#whyus"));

renderFaq(document.querySelector("#card-accordion"));
